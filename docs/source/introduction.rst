...
Users can use this interface to configure the resources used for the entire training process and the underlying engine settings. Refer to :ref:`rlite_init` for more details.
...
As you can see, the code is very similar to single-process PyTorch programs. The train engine will automatically handle data parallel or sequence parallel if the argument is wrapped by ``rlite.NeedParallel``. See :ref:`train` for more details.
